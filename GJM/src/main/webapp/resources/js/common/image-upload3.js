...
function onImageListener() {
	...
	$(".image-show-ok").on("click", function() {
		...
		//图片上传
		ajaxUploadImage(data , imageType);
	});
		...
		success: function(result) {
			switch (result.code) {
				case 200:
					...
					html += '	<span class="image-item-label ' + type_class + '">' + type + '</span>';
					html += '	<span class="image-item-close" title="删除" data-src="' + result.data.image + '" data-id="">X</span>';
					html += '</div>';
					html += '';
					html += '';
