...
/*		html += '		<li style="' + (_val == "封面图片" ? "display:none" : "") + '">封面图片</li>';
		...
		html += '		<li style="' + (_val == "3D图片" ? "display:none" : "") + '">任务名片</li>';
		html += '	</ul>';
		html += '</div>';
		$(this).after(html);
...
function setImageURL(file, type) {
	...
	if ($(".image-show").length > 0) {
		...
		html += '			<ul>';
		html += '				<li>';
		html += '					<select id="image-show-change" class="image-show-change" style="padding: 6px;margin-right:30px;border-radius: 3px;border: 1px solid #ddd;">';
	...
	/*	html += '						<option value="effect">效果图片</option>';
		...
		html += '						<option value="3d">人物名片</option>';
		html += '					</select>';
		html += '				</li>';
		...
		html += '				<li class="image-show-icon image-show-ok" data-type="' + type + '"><i class="icon-ok"></i>&nbsp;名片上传</li>';
...
//			return;
//		}
	    var data = _image.cropper("getCroppedCanvas").toDataURL();
...
function ajaxUploadImage(data, imageType, type) {
	...
	$.ajax({
		...
		success: function(result) {
			var html = "";
			if(result.data.userImage != null && result.data.userImage != ""){
				$("#imageUrl").val(result.data.userImage);
				
				html +='<div class="image-item">';
		    	html +='<label class="image-item-add" for="house-image"><input type="file" id="house-image">' 
				html +='	<div><img class="image-item-img image-card" onmouseover="getImageMouse()" onmouseout="leverImage()" src="'+ result.data.userImage+'"></div>';
