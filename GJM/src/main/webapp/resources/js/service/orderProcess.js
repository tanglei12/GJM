	...
	$.ajax({
	    ...
	    success: function(result) {
	    	...
	    	$.each(result.maintenanceTracksList, function(index, items) {
	    		...
	    		//循环维修数据
	    		if(dates == ""){
	    			if(items.mtk_state == 'no'){
    					var tt = "服务中";
    				}
    				if(items.mtk_state == 'yes'){
    					var tt = "服务完成";
    				}
    				if(items.mtk_state == 'error'){
    					var tt = "服务中止";
    				}
	    			dates.push(new Date(items.mtk_end_time).format("yyyy-MM-dd"));
	    			str+="<li><span class='date'>"+ new Date(items.mtk_end_time).format("yyyy-MM-dd") +"</span><span class='week'>"+ week(new Date(items.mtk_end_time).format("yyyy-MM-dd")) +"</span><span class='time'>"+ new Date(items.mtk_end_time).format("hh:mm:ss") +"</span><span class='text'>"+ items.mtk_spe_cir +"</span><span class='text'>"+ tt +"</span></li>";
	    		}else{
	    			for (var i = 0; i < dates.length; i++) {
		    			if(dates[i] == new Date(items.mtk_end_time).format("yyyy-MM-dd")){
			    			...
			    			str+="<li><span class='date' style='visibility: hidden;'>"+ new Date(items.mtk_end_time).format("yyyy-MM-dd") +"</span><span class='week' style='visibility: hidden;'>"+ week(new Date(items.mtk_end_time).format("yyyy-MM-dd")) +"</span><span class='time'>"+ new Date(items.mtk_end_time).format("hh:mm:ss") +"</span><span class='text'>"+ items.mtk_spe_cir +"</span><span class='text'>"+ tt +"</span></li>";
			    			state = 1;
		    			}
					}
	    			if(state == 0){
