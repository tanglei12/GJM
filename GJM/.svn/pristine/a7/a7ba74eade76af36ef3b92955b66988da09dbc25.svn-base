...
Fe.extend = function(d, b) {
	if (d && b && typeof (b) == "object") {
		...
		var a = [ "constructor", "hasOwnProperty", "isPrototypeOf",
		...
		onSearchComplete : function(w) {
			...
			if (b.getStatus() == BMAP_STATUS_SUCCESS) {
				...
				if (l == 1 && typeof w.getPoi(0).city == "undefined") {
					var f = 4;
					if (b && b._json && b._json.content
							&& b._json.content.length == 2) {
						f = b._json.content[1];
						p = '<b style="font-size:14px;">' + w.keyword + "</b>"
					} else {
						if (b
								&& b._json
								&& b._json.current_city
								&& b._json.current_city.name
								&& typeof b._json.current_city.level != "undefined") {
							f = b._json.current_city.level;
							if (b._json.current_city.name == "全国") {
								f = 4
							}
							p = '<span style="color:#00c">已切换至'
									+ b._json.current_city.name + "</span>";
					...
					for (var z = 0; z < l; z++) {
						...
						p += '	<div id="no_' + z + '">';
						p += '		<a href="javascript:void(0)" title="' + q
								+ '">' + x + "</a>";
						p += "		<p>" + t + "：" + u;
						n ? p += "		<br/>电话：" + n : null;
						p += "		<br/>坐标：" + A;
				...
				if (w.getCityList().length > 0) {
					...
					if (g.length > 0) {
						...
						for (var z = 0; z < g.length; z++) {
							y += "<li><a onclick=\"searchInthisCity('"
									+ g[z].city
...
function addMarker(a) {
	var b = temp.pt[a];
	var d = new BMap.Icon("/resources/Plug-in/map/Images/markers.png",
			new BMap.Size(23, 25), {
...
function setCurrentCity(d) {
	if (!d.content || d.content.error == 0) {
		...
		clearLastResult();
		Fe.G("resultNum").innerHTML = "";
		Fe.G("txtPanel").innerHTML = '<span style="color:#00c">已切换至'
				+ d.content.cname + "</span>"
	}
}
function hidePop() {
			...
			function() {
				if (Fe.G("ZeroClipboardMovie_1")) {
					...
					if (baidu.browser.ie == 8) {
						baidu.dom.getParent(Fe.G("ZeroClipboardMovie_1")).style.top = (c.y + 25)
								+ "px";
						baidu.dom.getParent(Fe.G("ZeroClipboardMovie_1")).style.left = (c.x + 170)
								+ "px"
					} else {
						if (baidu.browser.ie < 8) {
							baidu.dom.getParent(Fe.G("ZeroClipboardMovie_1")).style.top = (c.y - 10)
									+ "px";
							baidu.dom.getParent(Fe.G("ZeroClipboardMovie_1")).style.left = (c.x + 10)
									+ "px"
						} else {
							baidu.dom.getParent(Fe.G("ZeroClipboardMovie_1")).style.top = (c.y + 6)
									+ "px";
							baidu.dom.getParent(Fe.G("ZeroClipboardMovie_1")).style.left = (c.x + 11)
									+ "px"
						}
					}
					clearInterval(a)
...
function CreateFlash(a, e, f, b, d) {
	var c = '<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,0,0" width="'
			+ f + '" height="' + b + '" id="' + a + '" align="middle">';
...
function searchByPoint(c) {
	...
	var a = temp.geoCoder;
	if (d[0] && d[0].split(".")[0].length > 5 && d[1]
		...
		.extend(
				Page.prototype,
				{
					...
					getPage : function() {
						...
						var a = new RegExp("[?&]?" + this.argName
								+ "=([^&]*)[&$]?", "gi");
						var b = c.match(a);
						this.page = RegExp.$1
					},
					createHtml : function() {
						var b = [], f = this.page - 1, e = this.page + 1;
						b.push('<p class="page">');
						if (f < 1) {
						} else {
						...
						if (this.page < this.pagecap) {
							if (this.page % this.pagecap == 0) {
								...
								var a = this.page - this.page % this.pagecap
										+ 1
							}
						...
						Fe
								.on(
										...
										function(d) {
											...
											if (a.config.closeEffect
													&& typeof (a.config.closeEffect) == "function") {
											...
											a.visible = false;
											if (a.config.close
													&& typeof (a.config.close) == "function") {
												a.config.close()
											}
											if (this.resizeTimer) {
												window
