...
! function(b) {
	...
	function h(e) {
		var a = e || window.event,
			...
			n = 0,
			q = 0;
		if (e = b.event.fix(a), e.type = "mousewheel", "detail" in a && (g = -1 * a.detail), "wheelDelta" in a && (g = a.wheelDelta), "wheelDeltaY" in a && (g = a.wheelDeltaY), "wheelDeltaX" in a && (d = -1 * a.wheelDeltaX), "axis" in a && a.axis === a.HORIZONTAL_AXIS && (d = -1 * g, g = 0), c = 0 === g ? d : g, "deltaY" in a && (g = -1 * a.deltaY, c = g), "deltaX" in
			a && (d = a.deltaX, 0 === g && (c = -1 * d)), 0 !== g || 0 !== d) {
			...
			if (h = Math.max(Math.abs(g), Math.abs(d)), (!p || p > h) && (p = h, l.settings.adjustOldDeltas && "mousewheel" === a.type && 0 === h % 120 && (p /= 40)), l.settings.adjustOldDeltas && "mousewheel" === a.type && 0 === h % 120 && (c /= 40, d /= 40, g /= 40), c = Math[1 <= c ? "floor" : "ceil"](c / p), d = Math[1 <= d ? "floor" : "ceil"](d / p), g = Math[1 <= g ? "floor" : "ceil"](g /
					p), l.settings.normalizeOffset && this.getBoundingClientRect) a = this.getBoundingClientRect(), n = e.clientX - a.left, q = e.clientY - a.top;
			return e.deltaX = d, e.deltaY = g, e.deltaFactor = p, e.offsetX = n, e.offsetY = q, e.deltaMode = 0, f.unshift(e, c, d, g), m && clearTimeout(m), m = setTimeout(u, 200), (b.event.dispatch || b.event.handle).apply(this, f)
		}
	}
	...
	if (b.event.fixHooks)
		for (var n = t.length; n;) b.event.fixHooks[t[--n]] = b.event.mouseHooks;
	var l = b.event.special.mousewheel = {
		version: "3.1.12",
		setup: function() {
			if (this.addEventListener)
				for (var e = q.length; e;) this.addEventListener(q[--e], h, !1);
		...
		teardown: function() {
			if (this.removeEventListener)
				for (var e = q.length; e;) this.removeEventListener(q[--e],
					h, !1);
		...
		getLineHeight: function(e) {
			...
			return a.length || (a = b("body")), parseInt(a.css("fontSize"), 10) || parseInt(e.css("fontSize"), 10) || 16
		},
...
(function(b, h, u, m) {
	...
	function t(a, f) {
		...
		this.zoomStep = .25;
		this.zoomSpeed = .15;
		this.dragOutOfBoundsY = this.dragOutOfBoundsX = this.currentPosY = this.currentPosX = this.targetPosY =
			this.targetPosX = this.defaultPosY = this.defaultPosX = 0;
		this.intertia = .9;
		...
		this.didDoubleTap = !1;
		this.lastZoom = this.initialZoom = this.pinchDelta = this.initialPinchDistance = this.lastTouchY = this.lastTouchX =
			0;
		this.pinchZooming = !1;
	...
	b(h).height();
	(function() {
		b(u).on("mousemove", function(a) {
			k != m && k.handle_event(a)
		});
		b(u).on("mouseup", function(a) {
			k != m && (k.handle_event(a), k = m)
		});
		b(h).on("touchmove", function(a) {
		...
		b(h).on("touchend", function(a) {
			k != m && (k.handle_event(a), k = m)
		});
		b(h).on("resize", function() {
			b(h).width();
			b(h).height()
		})
	})();
	...
	t.prototype = {
		init: function() {
			...
			a.options.starting_in_fullscreen && (a.is_fullscreen = !0, a.obj.addClass("ndd-uberzoom-container-fullscreen"));
			...
			a.img.onload = function() {
				...
				a.options.fullscreen && (a.obj_interface.append('<div class="ndd-uberzoom-fullscreen"></div'), a.obj_fullscreen = a.obj_interface.find(".ndd-uberzoom-fullscreen"),
					a.obj_fullscreen.css({
						width: 44,
						height: 44,
						left: "100%",
						top: "100%",
				...
				b(h).on("resize", function(b) {
					a.handle_event(b)
				});
				b(a.obj).on("touchstart", function(b) {
				...
				a.options.startInFullscreen && a.toggle_fullscreen()
			};
		...
		schedule_hide_ui: function() {
			var a = this;
			a.ui_visible && (clearTimeout(a.ui_hide_timeout), a.ui_hide_timeout =
				setTimeout(function() {
					a.hide_ui()
				}, 1E3))
		},
		toggle_fullscreen: function() {
			if (this.is_fullscreen) this.obj.remove();
				...
				var a = this.options;
				a.startInFullscreen = !1;
				a.starting_in_fullscreen = !0;
				a.width = "100%";
				a.height = "100%";
		...
		handle_event: function(a) {
			if (!("mousewheel" == a.type && this.currentZoom == this.maxZoom && 0 < a.deltaY || "mousewheel" == a.type && this.currentZoom == this.minZoom &&
					0 > a.deltaY))
				if (a.stopPropagation(), a.preventDefault(), "mousewheel" != a.type && !n && (b(a.target).hasClass("ndd-uberzoom-fullscreen") || b(a.target).parent().hasClass("ndd-uberzoom-fullscreen") || b(a.target).hasClass("ndd-uberzoom-navigator") || b(a.target).hasClass("ndd-uberzoom-navigator-image") || b(a.target).hasClass("ndd-uberzoom-navigator-window"))) "mousedown" == a.type && (l = !0, b(a.target).hasClass("ndd-uberzoom-navigator") || b(a.target).hasClass("ndd-uberzoom-navigator-image") || b(a.target).hasClass("ndd-uberzoom-navigator-window")) &&
					(this.navigator_window_start_dragging(a.pageX, a.pageY), this.navigator_dragging = !0), "mouseup" == a.type && (l = !1, (b(a.target).hasClass("ndd-uberzoom-fullscreen") || b(a.target).parent().hasClass("ndd-uberzoom-fullscreen")) && this.toggle_fullscreen()), "mousemove" == a.type && (l = !0, this.navigator_dragging && this.navigator_window_drag(a.pageX, a.pageY)), "touchstart" == a.type && 1 == a.originalEvent.touches.length && (this.show_ui(), l = !0, b(a.target).hasClass("ndd-uberzoom-navigator") || b(a.target).hasClass("ndd-uberzoom-navigator-image") ||
						b(a.target).hasClass("ndd-uberzoom-navigator-window")) && (this.navigator_window_start_dragging(a.originalEvent.touches[0].pageX, a.originalEvent.touches[0].pageY), this.navigator_dragging = !0), "touchmove" == a.type && 1 == a.originalEvent.touches.length && (this.show_ui(), l = !0, this.navigator_dragging && this.navigator_window_drag(a.originalEvent.touches[0].pageX, a.originalEvent.touches[0].pageY)), "touchend" == a.type && 0 == a.originalEvent.touches.length && (this.schedule_hide_ui(), l = !1, (b(a.target).hasClass("ndd-uberzoom-fullscreen") ||
						b(a.target).parent().hasClass("ndd-uberzoom-fullscreen")) && this.toggle_fullscreen());
				...
				"touchmove" == a.type && 2 == a.originalEvent.touches.length && (this.show_ui(), n = !0, f = a.originalEvent.touches[0].screenX, c = a.originalEvent.touches[0].screenY, d = a.originalEvent.touches[1].screenX, e = a.originalEvent.touches[1].screenY, this.pinchDelta = Math.sqrt(Math.pow(c - e, 2) + Math.pow(f - d, 2)) - this.initialPinchDistance, this.pinchDelta = 7 * this.pinchDelta / Math.sqrt(Math.pow(this.contentWidth,
					2) + Math.pow(this.contentHeight, 2)), f = a.originalEvent.touches[0].pageX - this.obj.offset().left, c = a.originalEvent.touches[0].pageY - this.obj.offset().top, d = a.originalEvent.touches[1].pageX - this.obj.offset().left, e = a.originalEvent.touches[1].pageY - this.obj.offset().top, this.pinchZoomOffsetX = (f + d) / 2, this.pinchZoomOffsetY = (c + e) / 2, this.pinch_zoom(this.pinchZoomOffsetX, this.pinchZoomOffsetY));
				"touchstart" == a.type && (this.show_ui(), this.dragging = n = !0, this.didDoubleTap ? (f = a.originalEvent.touches[0].pageX - this.obj.offset().left,
					c = a.originalEvent.touches[0].pageY - this.obj.offset().top, this.currentZoom < this.maxZoom / 2 ? this.zoom_in(f, c, !0) : this.zoom_out(f, c, !0), this.didDoubleTap = !1) : this.start_moving(a.originalEvent.touches[0].screenX, a.originalEvent.touches[0].screenY));
				"touchend" == a.type && (this.schedule_hide_ui(), n = !1, this.stop_moving(), this.pinchZooming && (this.pinchZooming = !1, this.currentZoom > this.maxZoom ? this.zoom_in(this.pinchZoomOffsetX, this.pinchZoomOffsetY, !0) : this.currentZoom < this.minZoom && this.zoom_out(this.pinchZoomOffsetX,
		...
		navigator_window_start_dragging: function(a, b) {
			if (!(a > this.obj_nav_window.offset().left && a < this.obj_nav_window.offset().left + this.nav_window_width && b > this.obj_nav_window.offset().top && b < this.obj_nav_window.offset().top + this.nav_window_height)) {
				...
				var c =
					this.contentWidth * this.currentZoom / this.navigatorWidth,
		...
		zoom_in: function(a, b, c) {
			...
			this.targetPosX = this.currentPosX - (a - this.currentPosX) / (this.contentWidth * this.currentZoom) * this.contentWidth * (this.targetZoom -
				this.currentZoom);
		...
		zoom_out: function(a, b, c) {
			...
			b = (b - this.currentPosY) / (this.contentHeight * this.currentZoom);
			this.targetPosX = this.currentPosX - (a - this.currentPosX) /
				(this.contentWidth * this.currentZoom) * this.contentWidth * (this.targetZoom - this.currentZoom);
			this.targetPosY = this.currentPosY - (this.targetZoom - this.currentZoom) * this.contentHeight * b;
			this.constrain_target_position();
			this.apply_zoom()
		},
		apply_zoom: function() {
			...
			a.redraw();.025 <
				Math.abs(a.currentZoom - a.targetZoom) || .5 < Math.abs(a.currentPosX - a.targetPosX) || .5 < Math.abs(a.currentPosY - a.targetPosY) ? a.zoomTimeout = setTimeout(function() {
					a.apply_zoom()
		...
		constrain_target_position: function() {
			var a = -this.contentWidth * this.targetZoom + this.frameWidth,
				b = -this.contentHeight *
				this.targetZoom + this.frameHeight;
		...
		constrain_current_position: function() {
			var a = -this.contentWidth * this.currentZoom + this.frameWidth,
				b = -this.contentHeight * this.currentZoom + this.frameHeight;
			this.dragOutOfBoundsY = this.dragOutOfBoundsX = 0;
			0 < this.currentPosX && (this.currentPosX = 0);
			0 < this.currentPosY && (this.currentPosY = 0);
			this.currentPosX < a && (this.currentPosX =
				a);
			this.currentPosY < b && (this.currentPosY = b)
		},
		redraw: function() {
			this.options.rubberband || this.constrain_current_position();
		...
		redraw_navigator: function() {
			if (this.options.navigator) {
				var a = this.contentWidth * this.currentZoom / this.navigatorWidth,
					...
					d = (this.contentHeight *
						this.currentZoom - this.frameHeight - this.currentPosY) / a;
