...
function loadBaseData(data) {
    ...
    var html = "";
    html += '	<span><label>地址：</label>' + returnValue(contractVo.house_address) + '</span>';
...
function loadAuditingData(data) {
    ...
    // 房屋地址
    $("#houseAddress").val(contractInfo.house_address).attr("data-code", contractInfo.hi_code);

    if (!isEmpty(cancelContract) && cancelContract.cco_state == "取消") {
        cancelContract = "";
    }
    ...
    if (isEmpty(cancelContract)) {
        ...
        $.each(payTypeList, function (index, data) {
            var _name = (data.contractType_Name == '退租' ? "强退" : data.contractType_Name);
            var html = "";
            html += '<label class="common-borderbox ' + (index == 0 ? 'common-borderbox-checked' : '') + '">';
            html += '	<input type="radio" name="cancelType" value="' + data.contractType_Name + '" ' + (index == 0 ? 'checked' : '') + '>' + _name + '';
            html += '</label>';
        ...
        // 经办人
        $("#cco_peopleName").val(returnValue($.cookie("em_name")));
        // 经办日期
        ...
        /* var html = "";
         ...
         html += '	<input type="radio" name="cancelType" value="' + cancelContract.cco_applicationType + '" checked>' + (cancelContract.cco_applicationType == '退租' ? "强退" : cancelContract.cco_applicationType) + '';
         html += '</label>';
         $("#contractTypeBox").html(html);*/

        $.each(payTypeList, function (index, data) {
            // var _name = (cancelContract.cco_applicationType == '退租' ? "强退" : data.contractType_Name);
            var _name = (data.contractType_Name == '退租' ? "强退" : data.contractType_Name);
            var html = "";
            html += '<label class="common-borderbox ' + (cancelContract.cco_applicationType == data.contractType_Name ? 'common-borderbox-checked' : '') + '">';
            html += '	<input type="radio" name="cancelType" value="' + data.contractType_Name + '" ' + (cancelContract.cco_applicationType == data.contractType_Name ? 'checked' : '') + '>' + _name + '';
            html += '</label>';
            $("#contractTypeBox").append(html);
        });
    ...
    $("#cco_handleDate").on("focus", function () {
        ...
        WdatePicker({
            minDate: returnDate(contractInfo.contractObject_Date),
            maxDate: returnDate(_max),
...
function loadPaywayData(data) {
    ...
    if (contractInfo.contractObject_Type == '租赁合同') {
        bill_code = data.contractOrder.bco_code;
        ...
        bill_html += '	</div>';
        bill_html += '	<div class="item-table item-table-body" style="max-height: 320px;">';
        $.each(data.contractBillList, function (index, data) {
            var bcb_state = returnBillState(data.bcb_state);
            bill_html += '<ul style="' + (index % 2 == 0 ? 'background: #f8f8f8' : 'background: #E0EFF7') + '">';
            bill_html += '	<li style="width: 10%">' + returnValue(data.bcb_cycle) + '</li>';
            bill_html += '	<li style="width: 10%">' + returnFloat(data.bcb_repayment) + '</li>';
            bill_html += '	<li style="width: 10%">' + returnFloat(data.bcb_realPayment) + '</li>';
            bill_html += '	<li style="width: 20%">' + returnDate(data.bcb_repaymentDate) + '</li>';
            bill_html += '	<li class="' + bcb_state.style + '" style="width: 10%">' + bcb_state.text + '</li>';
            bill_html += '</ul>';
        });
        bill_html += '	</div>';
        bill_html += '</div>';
...
function applySubmit() {
    ...
    $.ajax({
        ...
        switch (result.code) {
            case 200:
                $.jBox.prompt("保存成功<br>请联系主管进行合约审核", "提示", "success", {
                    closed: function () {
                        // 关闭页面
