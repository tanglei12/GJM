...
public class PinYin2Abbreviation {
    ...
    /**
     * 获取名称首字母跟输入条件匹配上得名称集合
     * @param queryStr
     * @param nameList
     * @return
     */
    public static List<String> getStartChar(String queryStr, List<String> nameList){
		...
		for(String name : nameList){
			...
			boolean hasNotRegex = true;
			for(int i = 0; i < charArray.length; i++){
				String nameStartChar = cn2py(name.trim().substring(i, i+1));
				if(!String.valueOf(charArray[i]).equalsIgnoreCase(nameStartChar)){
					hasNotRegex = false;
					break;
				}
			}
    ...
    public static void main(String[] args) throws Exception {  
        ...
        System.out.println(cn2py("网友康年国际公寓"));
    }  
}
