...
//		console.info(this.uploadFile);
	...
	funUploadFile : function(file){
		...
		formdata.append("type", $("#uploadForm").attr("data-type"));
...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					// 创建带有拖动的html
					html += '<form id="uploadForm" action="'+para.url+'" method="post" enctype="multipart/form-data" data-type="'+ para.type +'">';
...
//		            html += '					<div class="webuploader_pick">继续选择</div>';
		            html += '					<div class="upload_tips"></div>';
			...
			this.funFilterEligibleFile = function(files){
				...
				for (var i = 0, file; file = files[i]; i++) {
					...
					if(newStr.split(".")[0] != null){
						...
						if(jQuery.inArray(type.toLowerCase(), para.fileType) > -1){
							// 类型符合，可以上传
							if (file.size >= para.fileSize) {
								$(".upload_tips").text('您这个"'+ file.name +'"文件大小过大').show().tipsTimeout();;
							} else {
								// 在这里需要判断当前所有文件中
								arrFiles.push(file);	
							}
						}else{
							$(".upload_tips").text('您这个"'+ file.name +'"上传类型不符合').show().tipsTimeout();;	
						}
					}else{
						$(".upload_tips").text('您这个"'+ file.name +'"没有类型, 无法识别').show().tipsTimeout();;	
					}
				}
				return arrFiles;
			};
			...
			this.addEvent = function(){
				...
				$(".upload_btn").bind("click", function(e){
					...
					if(ZYFILE.funReturnNeedFiles().length <= 0){
						$(".upload_tips").text("请先选中文件再点击上传").show().tipsTimeout();
					}else{
						if(ZYFILE.funReturnNeedFiles().length > para.fileCount){
							$(".upload_tips").text("最多只能上传"+ para.fileCount +"张图片").show().tipsTimeout();
						} else {
							$("#fileSubmit").click().attr("disabled", "disabled");
						}
					}
	            });
