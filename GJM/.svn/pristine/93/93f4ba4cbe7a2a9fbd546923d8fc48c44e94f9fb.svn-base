...
<!-- 底部 css样式 -->
<link href="/resources/Plug-in/map/Css/public.css" rel="stylesheet" type="text/css" />
<script type="text/javascript" src="/resources/Plug-in/map/Js/xx.js"></script>
<script type="text/javascript" src="/resources/Plug-in/map/Js/tangram.js"></script>
<script type="text/javascript" src="/resources/Plug-in/map/Js/mapext.js"></script>
...
<body onresize="mapResize()">
	<input type="hidden" id="in_hi_address" value="${hi_address }">
	<input type="hidden" id="in_xyz" value="${xyz }">
	<div class="content">
		<div class="LogoCon clear">
			<div class="logo"></div>
			<div class="search map_search">
				<input type="text" class="text" id="localvalue"   callback="beginsearch(l_local)"/>
				<input class="button" type="button" value="搜索" id="localsearch" />
				<label class="pointLabel" for="pointLabel">
					<input type="checkbox" onfocus="this.blur()" id="pointLabel" />坐标反查
				</label>
			...
			<div style="width:400px;position:relative;float:right;">
				...
				<input type="text" readonly id="pointInput" style="display:inline-block;background:#EBEBE4;border:#7F9DB9 solid 1px;color:#555;width:160px;height:30px;line-height:30px;font-size:14px;font-weight:700" />
				<span id="copyButton"><input type="button" value="确定" id="Point" onclick="latitude();" style="margin-left:8px;margin-top:-8px;float:right" /></span>
				<span id="copyMessage" style="width:50px;position:absolute;top:5px;right:175px;*right:170px;color:#f00;display:none">复制成功</span>
		...
		<div class="dt_nav">
			...
			<ul class="nav">
				<li>
					<div class="l">
						<b id="curCity"></b>
						<span>[<a id="curCityText" onclick="showPop()">更换城市</a>]</span>
					</div>
					<span class="r"></span>
				</li>
				<li>
					<div class="l">
						<b>当前层级：<span id="ZoomNum"></span>级</b>
					</div>
					<span class="r"></span>
				</li>
			</ul>
		...
		<div id="wrapper">
			...
			<!--右侧地图Info begin-->
			<div id="MapInfo"><div id="txtPanel">
				<h3>功能简介：</h3>
				<p class="tip_info">
					1、支持地址 精确/模糊 查询；<br/>
					2、支持POI点坐标显示、复制；<br/>
					3、坐标鼠标跟随显示；<br/>
					4、<font color="red">支持坐标查询(需要将坐标反查框勾选)；</font><br/>
				</p>
				<h3>使用说明：</h3>
				<p class="tip_info">
					1、获取坐标并复制：<br/>
					<span class="tip_info_em">1)、在搜索框中搜索关键词后，左侧列表中会有该点的坐标，点击该条信息或地图上该点，都会将坐标显示在地图右上角的Input框中,然后点击复制按钮，该点坐标就复制成功了;<br />2)、在地图上用鼠标左键单击地图，就能将该点坐标显示在地图右上角的Input框中,然后点击复制按钮，该点坐标就复制成功了;</span>
					2、坐标反查：<br/>
					<span class="tip_info_em">1)、先勾选住 搜索框后面的 <font color="red">坐标反查框</font><br />2)、输入一个正确的坐标(比如：116.307629,40.058359)，点击按钮 <b>百度一下</b>，就能将该点显示在地图上、切换地图，<font color="red">如果解析成功，还能返回一个地址</font></span>
				</p>
			</div>
		</div>
		<!--右侧地图Info end-->
...
</body><script>loadBody();</script>
<script>
	function myfun(){
		window.document.getElementById("curCity").innerHTML="重庆市";
		window.document.getElementById("localvalue").value="重庆市";
		window.document.getElementById("localsearch").click();
		var hi_address = window.document.getElementById("in_hi_address").value;//propertyInfo_Id
		var xyz = window.document.getElementById("in_xyz").value;//propertyInfo_Id
// 		alert(address);
// 		var objarray=address.length;
// 		var chestr="";
// 		for (i=0;i<objarray;i++)
// 		{
// 		  if(address[i].checked == true)
// 		  {
// 		   chestr = address[i].value;
// 		  }
// 		}
// 		var hi_address = window.parent.document.getElementById('pr'+window.parent.document.getElementById("in_hi_address").value).innerHTML;//propertyInfo_Id

		if(hi_address.indexOf("重庆") > -1){
			if(hi_address.indexOf("重庆市") > -1){
				document.getElementById('localvalue').value = hi_address;
			}else{
				hi_address=hi_address.replace("重庆","重庆市");
				document.getElementById('localvalue').value = hi_address;
			}
		}else{
			document.getElementById('localvalue').value = "重庆市"+hi_address;
		}
//        if(null != xyz && "" != xyz && "undefined" != xyz){
//            $("#pointLabel").click();
//        }
        document.getElementById('pointInput').value = "undefined" == xyz || null == xyz ? "" : xyz;
		document.getElementById("localsearch").click();
	}
	window.onload=myfun;
	function latitude(){
		var latitude = document.getElementById('pointInput').value;
		var updEl = window.parent.document.getElementById('updPropertyInfo_coordinate');
		if(updEl != "undefined" && updEl != null){
            updEl.value=latitude;
        }
        var addEl = window.parent.document.getElementById('addPropertyInfo_coordinate');
		if(addEl != "undefined" && addEl != null){
		    addEl.value=latitude;
        }
        var bzEl = window.parent.document.getElementById('biaozhu');
		if(bzEl != "undefined" && bzEl != null){
            bzEl.value=latitude;
        }
        var house_type = getUrlParam("house_type");
        if(house_type == 2){
            var outEl = window.parent.document.getElementById("out_point");
            if(outEl != "undefined" && outEl != null){
                outEl.value=latitude;
            }
        } else if(house_type == 3){
            var inEl = window.parent.document.getElementById("in_point");
