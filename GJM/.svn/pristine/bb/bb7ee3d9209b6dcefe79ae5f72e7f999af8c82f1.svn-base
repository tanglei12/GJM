	    ...
	    success: function(result) {
	    	...
	    	if(result != "1"){
	    		...
	    		$.each(result.userCenterEmployeeTable, function(idx, item) {
	    			...
	    			html+="<tr class='tr' id='data_contents'><td><input name='chickes' type='checkbox' id='"+item.em_id+"'/></td><td>"+(idx+1)+"</td><td><a href='javascript:;' onclick='hrefClick(this)' data-type='/user/userUpdate?id="+ item.em_id +"'>"+item.em_name+"</a></td><td>"+sex+"</td><td>"+item.em_position+"</td><td>"+item.em_phone+"</td><td>"+item.em_address+"</td><td>"+item.em_account+"</td><td>"+ state +"</td><td>"+ item.em_post +"</td></tr>";
	    	    });
...
function hrefClick(ids){
	window.parent.href_mo($(ids).attr("data-type"),"修改用户","用户管理");
}
