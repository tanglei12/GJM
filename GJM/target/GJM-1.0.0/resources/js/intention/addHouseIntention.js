...
/** 房源跟进提交 */
function fangyuangenjinsubmit(tipnum,obj){
    if(tipnum == 5){
	...
	+ "&hi_houseT="+$("#fangwuting").val()+ "&hi_houseW="+$("#fangwuwei").val() + "&hi_code="+$("#hicode").val();
...
/** 房源实勘提交qs */
function fangyuanshikansubmit(tipnum,obj){
    if(tipnum == 5){
        $.jBox.tip("该房源已跟进完成，不能修改！");
        return;
    }
	...
	+"&hi_situation="+$("#zhuangxiuqingkuang").val()+"&phi_style="+$("#phi_style").val()+"&his_id="+$("#housepinpai").val()
	...
	+"&new_buildType="+$("#buildtype").val()+"&path="+img_path;
	$.ajax({
		type:"POST",
		url:"/intention/addInitqs",
		data:count,
...
/** 房源定价提交qs */
function fangyuandingjiasubmit(tipnum,obj){
    if(tipnum == 5){
        $.jBox.tip("该房源已跟进完成，不能修改！");
        return;
    }
...
/** 房源存房提交qs */
function fangyuancunfangsubmit(tipnum,obj){
