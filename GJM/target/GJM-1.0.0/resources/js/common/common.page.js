...
;(function ($) {
    ...
    $.table = function (opts) {
        ...
        opts.targetId = opts.targetId || document.body;
        ...
        if (opts.filterWhere) {
            ...
            html += '				<button class="where-head-btn next-bg" name="filter-add" style="margin-right: 10px;"><i class="fa-plus"></i>添加</button>';
            html += '				<button class="where-head-btn error-bg" name="filter-clean-all"><i class="fa-refresh"></i>重置</button>';
        ...
        if (opts.filterDate) {
            ...
            html += '				<option value="toDay">今日</option>';
            html += '				<option value="toWeek">本周</option>';
            html += '				<option value="toMonth">本月</option>';
        ...
        html += '		<div class="custom-table-head">';
        ...
        html += '		</div>';
        html += '		<div class="custom-table-body custom-scroll">';
        html += '		    <table>';
        html += '		        <thead>';
        html += '		    	    <tr>';
        ...
        html += '   </div>';
        html += '</div>';
        var tableBox = $(html).appendTo(opts.targetId);
    ...
    $.table.loadData = function () {
        ...
        opts.ajaxParams.data = $.extend({}, opts.ajaxParams.data, $.table.getRequestParam());
            ...
            $.each(list, function (index, data) {
                ...
                $.each(opts.listParams, function (index1, item) {
                    ...
                    if (typeof item.name === "string" && item.name.indexOf("{") > -1 && item.name.indexOf("}") > -1) {
                        ...
                        if (!isEmpty(name1)) {
                            html1 = '<label class="' + returnValue(state.style) + '">' + returnValue(state.text) + '</label>';
                        }
                        ...
                        to_boo = !isEmpty(state.text);
                        if (!isEmpty(name2)) {
                            html2 = '<label class="' + returnValue(state.style) + '">' + returnValue(state.text) + '</label>';
                        }
                        var to = to_boo ? text.substring(text.indexOf("{") + 1, text.indexOf("}")) : "";

                        body_html += '<td name="' + name1 + (isEmpty(name2) ? "" : "_" + name2) + '">' + html1 + to + html2 + '</td>';
                    }
                    else {
    ...
    $.table.getRequestParam = function () {
        var opts = $.table.defaults;
        // 参数
     ...
     * @param i
     */
    $.table.getTableContent = function (i) {
        return $.table.getBox().find('.list-table-content');
    };
     ...
     * @returns {*}
     */
    $.table.getCurrentItemData = function (obj) {
        return $(obj).parents(".list-content-item").find("[name=table-checkbox]").data("data");
    };
    ...
    $.popupBox = function (param) {
        param = $.extend({
            target: "",
            data: {},
            theme: "default", // default：默认主题、info：详情
            width: "75%",
            buttons: {"close": "关闭", "refresh": "刷新"},
            done: function (box, _data) {},
            close: function () {}
        }, param);

        var _box = {};
        var _having = $(".custom-popup").length == 1;

        _box.popup = _having ? $(".custom-popup") : $('<div class="custom-popup popup-' + param.theme + '"></div>').appendTo($(param.target || "body"));
        _box.option = _having ? _box.popup.find(".popup-option") : $('<div class="popup-option"></div>').appendTo(_box.popup);

        _box.head = _having ? _box.popup.find(".popup-head") : $('<div class="popup-head"></div>').appendTo(_box.popup);
        _box.nav = _having ? _box.popup.find(".popup-nav") : $('<div class="popup-nav"></div>').appendTo(_box.popup);
        _box.main = _having ? _box.popup.find(".popup-main") : $('<div class="popup-main custom-scroll"></div>').appendTo(_box.popup);
        _box.foot = _having ? _box.popup.find(".popup-foot") : $('<div class="popup-foot"></div>').appendTo(_box.popup);
        ...
        if (param.theme != "info") _box.nav.hide();
        ...
        _box.option.html(html);

        // 【样式】宽
        _box.popup.css({width: param.width});

        // 【事件】关闭
        _box.popup.find("[name=popup-close]").off().on("click", function () {
            $.popupBoxClose();
            param.close();
        });

        // 【事件】刷新
        _box.popup.find("[name=popup-refresh]").off().on("click", function () {
            $(this).find("i").addClass("animation-spin");
            param.done(_box, param.data);
        });

        // 【事件】特效
        _box.popup.off().on({
            "mouseover": function () {
                // html_data.box.css("background", "rgba(255, 255, 255, 1)");
            },
            "mouseleave": function () {
                // html_data.box.css("background", "rgba(255, 255, 255, 0.7)");
            }
        });
     ...
     * 弹出层-刷新
     */
    $.popupRefreshAnimation = function () {
        $(".custom-popup").find("[name=popup-refresh] > i").addClass("animation-spin");
    };
    ...
    $.popupSideView = function (param) {
        ...
        var custom_popup = $(".custom-popup");
        var popup_side = $(".custom-popup-side");

        var init = function () {
                if (custom_popup.length < 1) return;
                if (popup_side.length < 1) {
                    var side_style = '';
                    var main_style = '';
                    switch (param.mode) {
                        case 'top':
                            side_style = 'top:0;width:100%;min-height:' + param.wh;
                            main_style = 'top:-100%;min-height:' + param.wh;
                            break;
                        case 'bottom':
                            side_style = 'bottom:0;width:100%;min-height:' + param.wh;
                            main_style = 'bottom:-100%;min-height:' + param.wh;
                            break;
                        case 'right':
                            side_style = 'right:0;height:100%;min-width:' + param.wh;
                            main_style = 'right:-100%;min-width:' + param.wh;
                            break;
                        case 'left':
                            side_style = 'left:0;height:100%;min-width:' + param.wh;
                            main_style = 'left:-100%;min-width:' + param.wh;
                            break;
                    }
                    var html =
                        '<div class="custom-popup-side">' +
                        '   <div class="popup-side-head" style="flex: 1;"></div>' +
                        '   <div class="popup-side-main" data-mode="' + param.mode + '" style="' + main_style + '">' +
                        ...
                        '   </div>' +
                        '</div>';
                    popup_side = $(html).appendTo(custom_popup);

                    var popup_side_main = popup_side.find(".popup-side-main");
                    var popup_side_option = popup_side.find(".popup-side-option");

                    // 初始化模式
                    switch (param.mode) {
                        case "top":
                            popup_side.css({flexDirection: "column-reverse"});
                            popup_side_option.css({top: "auto", bottom: "-30px", left: "50%", right: "auto"});
                            popup_side_main.animate({top: 0}, 300);
                            break;
                        case "bottom":
                            popup_side.css({flexDirection: "column"});
                            popup_side_option.css({top: "-30px", bottom: "auto", left: "50%", right: "auto"});
                            popup_side_main.animate({bottom: 0}, 300);
                            break;
                        case "right":
                            popup_side.css({flexDirection: "row"});
                            popup_side_option.css({top: "6%", bottom: "auto", left: "-10px", right: "auto"});
                            popup_side_main.animate({right: 0}, 300);
                            break;
                        case "left":
                            popup_side.css({flexDirection: "row-reverse"});
                            popup_side_option.css({top: "6%", bottom: "auto", left: "auto", right: "-30px"});
                            popup_side_main.animate({left: 0}, 300);
                            break;
                    }
                }

                // 关闭
                popup_side.find(".popup-side-head").off().on("click", function () {
                    if (param.close) param.close();
                    $.popupSideViewClose();
                });

                // 关闭
                popup_side.find("[name=popup-side-close]").off().on("click", function () {
                    if (param.close) param.close();
                    $.popupSideViewClose();
                });

                // 回调
                if (param.done) param.done(popup_side.find(".popup-side-content"));
            },
            self = {
                done: function () {
                    return this;
    ...
    $.popupSideViewClose = function () {
        var popup_side = $(".custom-popup-side");
        var popup_side_main = popup_side.find(".popup-side-main");
        switch (popup_side_main.attr("data-mode")) {
            case "bottom":
                popup_side_main.animate({bottom: "-100%"}, 300, function () {
                    popup_side.remove();
                });
                break;
            case "top":
                popup_side_main.animate({top: "-100%"}, 300, function () {
                    popup_side.remove();
                });
                break;
            case "right":
                popup_side_main.animate({right: "-100%"}, 300, function () {
                    popup_side.remove();
                });
                break;
            case "left":
                popup_side_main.animate({left: "-100%"}, 300, function () {
                    popup_side.remove();
                });
                break;
        }
