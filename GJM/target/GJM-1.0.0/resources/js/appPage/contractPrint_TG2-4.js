...
function loadHtml() {
    ...
    if(null != rentPlusStr && "" != rentPlusStr && undefined != rentPlusStr){
        ...
        for (var i = 0; i < rentArray.length; i++){
            ...
            //     html += '<p class="conInfo">第 ' + (i + 1) + ' 年租金：<label class="textUnderLine">&nbsp;' + rentArray[i] + '&nbsp;</label>元/月。</p>';// （总租金：<label class="textUnderLine">&nbsp;' + upperMoney.substring(0, (upperMoney.length - 1)) + '&nbsp;</label>整）
            // }
            if(payStyle == "年" && "磐谷" == returnValue($("#payYear1").text())){
                if(i < ymd.year){
                    var money = returnToUpperMoney(returnNumber(rentArray[i]) * 12);
                    html += '<p class="conInfo">第 ' + (i + 1) + ' 年固定租金回报：<label class="textUnderLine">&nbsp;' + rentArray[i] + '&nbsp;</label>元/月（年固定租金回报：<label class="textUnderLine">&nbsp;' + money.substr(0, money.indexOf("元整")) + '&nbsp;</label>元整）。</p>';
                } else {
                    var money = returnToUpperMoney(returnNumber(rentArray[i])*month + (returnNumber(returnNumber(rentArray[i]) / 30) * day));
                    html += '<p class="conInfo">第 ' + (i + 1) + ' 年固定租金回报：<label class="textUnderLine">&nbsp;' + rentArray[i] + '&nbsp;</label>元/月（年固定租金回报：<label class="textUnderLine">&nbsp;' + money.substr(0, money.indexOf("元整")) + '&nbsp;</label>元整）。</p>';
                }
            } else {
                html += '<p class="conInfo">第 ' + (i + 1) + ' 年租金：<label class="textUnderLine">&nbsp;' + returnNumber(rentArray[i]) + '&nbsp;</label>元/月。</p>';
            }
        }
        var totalmoney = returnToUpperMoney(returnNumber($("#totalRent").val()));
        html += '<p class="conInfo">合同总金额：<label class="textUnderLine">&nbsp;' + totalmoney.substr(0, totalmoney.indexOf("元整")) + '&nbsp;</label>元整（未包含押金）。</p>';
...
//		async: !isApp, // 是APP开启同步
    ...
    if(payStyle == "年" && "磐谷" == returnValue($("#payYear1").text())){
        $("#payYear1").show();
        $("#payYear1").parents("div.flex-item").css({"height": "48px"});
        $("#payYear1").text(returnYearCompany($("#payYear1").text()));
        $("#payYear2").show();
        $("#payYear2").parents("div.flex-item").css({"height": "48px"});
        $("#payYear2").text(returnYearCompany($("#payYear2").text()));
        $("#payYear3").show();
        $("#gjp1").hide();
        $("#pangu1").show();
        $("#gjp2").hide();
        ...
        $("#payYear1").hide();
        $("#payYear1").parents("div.flex-item").css({"height": "24px"});
        $("#payYear2").parents("div.flex-item").css({"height": "24px"});
        $("#payYear1").hide();
        $("#payYear2").hide();
        $("#payYear3").hide();
        $("#gjp1").show();
...
function returnYearCompany(year_pay) {
    var company = "";
    switch (year_pay){
        case "磐谷":
            company = "成都有家商业管理有限公司";
            break;
    }
    return company;
}
